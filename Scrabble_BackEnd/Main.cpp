...
int main()
{
	...
	while (1) {
		...
		if (bag->getSize() > 8 || !bag->hasLetters()) {
			...
			cout << "the bag size has :" << bag->getSize() << endl;
			cout << "the first Rack size has :" << players[0]->rack->getSize() << endl;
			cout << "the second Rack size has :" << players[1]->rack->getSize() << endl;
			system("pause");
