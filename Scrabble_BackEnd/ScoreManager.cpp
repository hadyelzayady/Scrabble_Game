...
int ScoreManager::computeMoveScore(int x[], int y[], char c[], int playCount) {
	...
	for (int i = 0; i < playCount; i++) {
		if (boardManager->board[x[i]][y[i]].bonusType == WordBonusX2) {
			W2++;
		}
		else if (boardManager->board[x[i]][y[i]].bonusType == WordBonusX3) {
			W3++;
		}
		else if (boardManager->board[x[i]][y[i]].bonusType == LetterBonusX2) {
			resultantScore += tileLookUp->getScore(c[i]) * 2;
		}
		else if (boardManager->board[x[i]][y[i]].bonusType == LetterBonusX3) {
