...
{																										//bag is left private in the class BagOfLetters
	...
	else if (bag->getSize() < 10 && bag->getSize() >0) return preEnd(estimatedRack, move, leave);
	else return midGame(turn,estimatedRack, move, leave);

}


int Heuristics::preEnd(Rack * estimatedRack, Move *move, Rack * leave)
{
	...
	{
		if (plays[i].get_Letter() == 'Q' || plays[i].get_Letter() == 'U' || plays[i].get_Letter() == 'Z' || plays[i].get_Letter() == 'X')
		{
			priority++;
		}
